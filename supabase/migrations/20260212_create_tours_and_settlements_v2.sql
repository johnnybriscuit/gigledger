-- Migration: Create Tours/Runs and Settlements feature (v2 - idempotent)
-- Date: 2026-02-12
-- Description: Add tour_runs, settlements tables and extend gigs/expenses for tour grouping
-- This version uses IF NOT EXISTS and DROP POLICY IF EXISTS for idempotency
...
-- ============================================================================
-- 6. RLS POLICIES for tour_runs (drop existing first)
-- ============================================================================
DROP POLICY IF EXISTS "Users can view own tours" ON tour_runs;
CREATE POLICY "Users can view own tours"
  ON tour_runs FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own tours" ON tour_runs;
CREATE POLICY "Users can insert own tours"
  ON tour_runs FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own tours" ON tour_runs;
CREATE POLICY "Users can update own tours"
  ON tour_runs FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own tours" ON tour_runs;
CREATE POLICY "Users can delete own tours"
  ON tour_runs FOR DELETE
  USING (auth.uid() = user_id);

-- ============================================================================
-- 7. RLS POLICIES for settlements (drop existing first)
-- ============================================================================
...
-- ============================================================================
DROP TRIGGER IF EXISTS update_tour_runs_updated_at ON tour_runs;
...
DROP TRIGGER IF EXISTS update_tour_dates_on_gig_change ON gigs;
