/**
 * Rate-limited magic link endpoint
 * Proxies to Supabase with rate limiting and optional Turnstile verification
 */
  ...
  try {
    ...
    if (process.env.EXPO_PUBLIC_ANTIBOT_ENABLED === 'true') {
      ...
      if (!verifyResponse.ok) {
        console.error('[send-magic-link] Turnstile verification request failed:', verifyResponse.status);
